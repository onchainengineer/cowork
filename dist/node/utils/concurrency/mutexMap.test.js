...
const mutexMap_1 = require("./mutexMap");
(0, bun_test_1.describe)("MutexMap", () => {
    (0, bun_test_1.test)("serializes concurrent operations on the same key", async () => {
        const mutex = new mutexMap_1.MutexMap();
        const key = "test";
        const events = [];
        ...
        (0, bun_test_1.expect)(results).toEqual([1, 2, 3, 4]);
        (0, bun_test_1.expect)(maxConcurrent).toBe(1); // Only one operation at a time
    ...
    (0, bun_test_1.test)("allows concurrent operations on different keys", async () => {
        const mutex = new mutexMap_1.MutexMap();
        const events = [];
        let concurrentCount = 0;
        let maxConcurrent = 0;
        ...
        (0, bun_test_1.expect)(results).toEqual([1, 2]);
        (0, bun_test_1.expect)(maxConcurrent).toBe(2); // Both should run concurrently
        // Both start before either ends
        (0, bun_test_1.expect)(events.slice(0, 2).sort()).toEqual(["key1-start", "key2-start"]);
    });
    (0, bun_test_1.test)("cleans up locks after operation completes", async () => {
        const mutex = new mutexMap_1.MutexMap();
        const key = "test";
        ...
        (0, bun_test_1.expect)(elapsed).toBeLessThan(10); // Should not wait
    });
    (0, bun_test_1.test)("cleans up locks after operation throws", async () => {
        const mutex = new mutexMap_1.MutexMap();
        const key = "test";
        let caught = false;
        try {
            ...
            (0, bun_test_1.expect)(e.message).toBe("intentional");
            caught = true;
        }
        (0, bun_test_1.expect)(caught).toBe(true);
        ...
        (0, bun_test_1.expect)(elapsed).toBeLessThan(10);
    });
    (0, bun_test_1.test)("preserves FIFO order for queued operations", async () => {
        const mutex = new mutexMap_1.MutexMap();
        const key = "test";
        const order = [];
        ...
        (0, bun_test_1.expect)(order).toEqual([1, 2, 3, 4]);
    });
});

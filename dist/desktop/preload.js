...
electron_1.contextBridge.exposeInMainWorld("api", {
    ...
    getIsRosetta: () => electron_1.ipcRenderer.invoke("unix:get-is-rosetta"),
    getIsWindowsWslShell: () => electron_1.ipcRenderer.invoke("unix:get-is-windows-wsl-shell"),
    // Register a callback for notification clicks (navigates to workspace)
    // Returns an unsubscribe function.
    onNotificationClicked: (callback) => {
        const listener = (_event, data) => callback(data);
        electron_1.ipcRenderer.on("unix:notification-clicked", listener);
        return () => {
            electron_1.ipcRenderer.off("unix:notification-clicked", listener);
        };
    },
});

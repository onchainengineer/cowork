...
exports.BashExecutionService = exports.DisposableProcess = void 0;
...
class BashExecutionService {
    ...
    executeStreaming(script, config, callbacks) {
        log_1.log.debug(`BashExecutionService: Executing streaming command in ${config.cwd}`);
        log_1.log.debug(`BashExecutionService: Script: ${script.substring(0, 100)}${script.length > 100 ? "..." : ""}`);
        ...
        log_1.log.debug(`BashExecutionService: Spawned process with PID ${child.pid ?? "unknown"}`);
        ...
        child.on("close", (code, signal) => {
            log_1.log.debug(`BashExecutionService: Process exited with code ${code ?? "null"}, signal ${signal ?? "none"}`);
        ...
        child.on("error", (error) => {
            log_1.log.error(`BashExecutionService: Process error:`, error);
        });
        return new DisposableProcess(child);
    }
}
exports.BashExecutionService = BashExecutionService;

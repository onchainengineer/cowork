...
const unix_global_agents_read_1 = require("./unix_global_agents_read");
const unix_global_agents_write_1 = require("./unix_global_agents_write");
const testHelpers_1 = require("./testHelpers");
const mockToolCallOptions = {
    toolCallId: "test-call-id",
    messages: [],
};
(0, bun_test_1.describe)("unix_global_agents_* tools", () => {
    (0, bun_test_1.it)("reads ~/.unix/AGENTS.md (returns empty string if missing)", async () => {
        const env_1 = { stack: [], error: void 0, hasError: false };
        try {
            const unixHome = __addDisposableResource(env_1, new testHelpers_1.TestTempDir("unix-global-agents"), false);
            const workspaceSessionDir = path.join(unixHome.path, "sessions", unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID);
            await fs.mkdir(workspaceSessionDir, { recursive: true });
            const config = (0, testHelpers_1.createTestToolConfig)(unixHome.path, {
                workspaceId: unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID,
                sessionsDir: workspaceSessionDir,
            });
            const tool = (0, unix_global_agents_read_1.createUnixGlobalAgentsReadTool)(config);
            // Missing file -> empty
            ...
            await fs.writeFile(agentsPath, `# ${unixChat_1.UNIX_HELP_CHAT_WORKSPACE_TITLE}\n${unixChat_1.UNIX_HELP_CHAT_WORKSPACE_NAME}\n`, "utf-8");
            const result = (await tool.execute({}, mockToolCallOptions));
            (0, bun_test_1.expect)(result.success).toBe(true);
            if (result.success) {
                (0, bun_test_1.expect)(result.content).toContain(unixChat_1.UNIX_HELP_CHAT_WORKSPACE_TITLE);
                (0, bun_test_1.expect)(result.content).toContain(unixChat_1.UNIX_HELP_CHAT_WORKSPACE_NAME);
            }
        }
        catch (e_1) {
            env_1.error = e_1;
    ...
    (0, bun_test_1.it)("refuses to write without explicit confirmation", async () => {
        ...
        try {
            const unixHome = __addDisposableResource(env_2, new testHelpers_1.TestTempDir("unix-global-agents"), false);
            const workspaceSessionDir = path.join(unixHome.path, "sessions", unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID);
            await fs.mkdir(workspaceSessionDir, { recursive: true });
            const config = (0, testHelpers_1.createTestToolConfig)(unixHome.path, {
                workspaceId: unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID,
            ...
            const tool = (0, unix_global_agents_write_1.createUnixGlobalAgentsWriteTool)(config);
            const agentsPath = path.join(unixHome.path, "AGENTS.md");
    ...
    (0, bun_test_1.it)("writes ~/.unix/AGENTS.md and returns a diff", async () => {
        const env_3 = { stack: [], error: void 0, hasError: false };
        try {
            const unixHome = __addDisposableResource(env_3, new testHelpers_1.TestTempDir("unix-global-agents"), false);
            const workspaceSessionDir = path.join(unixHome.path, "sessions", unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID);
            await fs.mkdir(workspaceSessionDir, { recursive: true });
            const config = (0, testHelpers_1.createTestToolConfig)(unixHome.path, {
                workspaceId: unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID,
    ...
    (0, bun_test_1.it)("rejects symlink targets", async () => {
        ...
        try {
            const unixHome = __addDisposableResource(env_4, new testHelpers_1.TestTempDir("unix-global-agents"), false);
            const workspaceSessionDir = path.join(unixHome.path, "sessions", unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID);
            await fs.mkdir(workspaceSessionDir, { recursive: true });
            const config = (0, testHelpers_1.createTestToolConfig)(unixHome.path, {
                workspaceId: unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID,
            ...
            const readTool = (0, unix_global_agents_read_1.createUnixGlobalAgentsReadTool)(config);
            const writeTool = (0, unix_global_agents_write_1.createUnixGlobalAgentsWriteTool)(config);
            const agentsPath = path.join(unixHome.path, "AGENTS.md");
...
//# sourceMappingURL=unix_global_agents.test.js.map
...
class WorkspaceService extends events_1.EventEmitter {
    ...
    constructor(config, historyService, partialService, aiService, initStateManager, extensionMetadata, backgroundProcessManager, sessionUsageService) {
        super();
        this.config = config;
    ...
    setExperimentsService(experimentsService) {
     ...
     */
    setSessionTimingService(sessionTimingService) {
    ...
    async getPostCompactionState(workspaceId) {
        ...
        const planPath = (0, planStorage_1.getPlanFilePath)(metadata.name, metadata.projectName, unixHome);
        ...
        const expandedPlanPath = unixHome.startsWith("~") ? (0, tildeExpansion_1.expandTilde)(planPath) : planPath;
    ...
    async create(projectPath, branchName, trunkBranch, title, runtimeConfig, sectionId) {
        ...
        if (projectPath === (0, unixChat_2.getUnixHelpChatProjectPath)(this.config.rootDir)) {
            return (0, result_1.Err)("Cannot create workspaces in the Chat with Unix system project");
        }
        ...
        try {
            ...
            const secrets = (0, secrets_1.secretsToRecord)(this.config.getProjectSecrets(projectPath));
            ...
            return (0, result_1.Ok)({ metadata: completeMetadata });
        }
        catch (error) {
            initLogger.logComplete(-1);
            const message = error instanceof Error ? error.message : String(error);
    ...
    async remove(workspaceId, force = false) {
        if (workspaceId === unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID) {
            return (0, result_1.Err)("Cannot remove the Chat with Unix system workspace");
        }
        ...
        try {
            ...
            if (metadataResult.success) {
                ...
                // Note: Lattice workspace deletion is handled by LatticeSSHRuntime.deleteWorkspace()
    ...
    async archive(workspaceId) {
        if (workspaceId === unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID) {
            return (0, result_1.Err)("Cannot archive the Chat with Unix system workspace");
        }
        try {
            const workspace = this.config.findWorkspace(workspaceId);
            if (!workspace) {
    ...
    async unarchive(workspaceId) {
        if (workspaceId === unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID) {
            return (0, result_1.Err)("Cannot unarchive the Chat with Unix system workspace");
        }
        try {
            const workspace = this.config.findWorkspace(workspaceId);
            if (!workspace) {
    ...
    async fork(sourceWorkspaceId, newName) {
        try {
            if (sourceWorkspaceId === unixChat_1.UNIX_HELP_CHAT_WORKSPACE_ID) {
                return (0, result_1.Err)("Cannot fork the Chat with Unix system workspace");
            }
            const validation = (0, workspaceValidation_1.validateWorkspaceName)(newName);
            if (!validation.valid) {
                return (0, result_1.Err)(validation.error ?? "Invalid workspace name");
            ...
            if ((0, runtime_1.isSSHRuntime)(sourceRuntimeConfig)) {
                return (0, result_1.Err)("Forking SSH workspaces is not supported. Create a new workspace instead.");
            }
            ...
            const secrets = (0, secrets_1.secretsToRecord)(this.config.getProjectSecrets(foundProjectPath));
            (0, runtimeFactory_1.runBackgroundInit)(runtime, {
                projectPath: foundProjectPath,
                branchName: newName,
            ...
            return (0, result_1.Ok)({ metadata, projectPath: foundProjectPath });
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    async replaceHistory(workspaceId, summaryMessage, options) {
        ...
        try {
            ...
            const typedSummaryMessage = { ...summaryMessage, type: "message" };
            if (session) {
    ...
    async executeBash(workspaceId, script, options) {
        ...
        try {
            ...
            try {
                ...
                const tempDir = __addDisposableResource(env_1, new tempDir_1.DisposableTempDir("unix-ipc-bash"), false);

    ...
    return (0, ai_1.tool)({
        ...
        execute: async ({ file_path, offset, limit }, { abortSignal: _abortSignal }) => {
            ...
            try {
                ...
                if (!(await (0, fileCommon_1.isPlanFilePath)(filePath, config))) {
                    const pathValidation = (0, fileCommon_1.validatePathInCwd)(filePath, config.cwd, config.runtime, [
                        config.runtimeTempDir,
                    ]);
                    if (pathValidation) {
                        // In plan mode, hint about the plan file path to help model recover
                        const hint = config.planFileOnly && config.planFilePath

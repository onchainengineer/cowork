    ...
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        for (const part of message.parts) {
            if (part.type !== "dynamic-tool")
                continue;
            if (!tools_1.FILE_EDIT_TOOL_NAMES.includes(part.toolName))
                continue;
...
function extractEditedFileDiffs(messages) {
    ...
    const results = [];
    for (let i = editOrder.length - 1; i >= 0 && results.length < attachments_1.MAX_EDITED_FILES; i--) {
...
function combineDiffs(filePath, diffs) {
    ...
    const combinedDiff = (0, diff_1.createPatch)(filePath, originalContent, content, "", "", { context: 3 });

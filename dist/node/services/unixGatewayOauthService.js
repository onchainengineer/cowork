...
exports.UnixGatewayOauthService = void 0;
const crypto = __importStar(require("crypto"));
const http = __importStar(require("http"));
const result_1 = require("../../common/types/result");
const unixGatewayOAuth_1 = require("../../common/constants/unixGatewayOAuth");
...
class UnixGatewayOauthService {
    ...
    async startDesktopFlow() {
        ...
        const authorizeUrl = (0, unixGatewayOAuth_1.buildAuthorizeUrl)({ redirectUri, state: flowId });
        ...
        log_1.log.debug(`Unix Gateway OAuth desktop flow started (flowId=${flowId})`);
    ...
    async cancelDesktopFlow(flowId) {
        ...
        if (!flow)
            return;
        log_1.log.debug(`Unix Gateway OAuth desktop flow cancelled (flowId=${flowId})`);
    ...
    startServerFlow(input) {
        ...
        const authorizeUrl = (0, unixGatewayOAuth_1.buildAuthorizeUrl)({ redirectUri: input.redirectUri, state });
        this.serverFlows.set(state, {
            state,
            expiresAtMs: Date.now() + DEFAULT_SERVER_TIMEOUT_MS,
        });
        log_1.log.debug(`Unix Gateway OAuth server flow started (state=${state})`);
    ...
    async handleDesktopCallback(input) {
        ...
        log_1.log.debug(`Unix Gateway OAuth callback received (flowId=${input.flowId})`);
    ...
    async handleCallbackAndExchange(input) {
        if (input.error) {
            const message = input.errorDescription
                ...
                : input.error;
            return (0, result_1.Err)(`Unix Gateway OAuth error: ${message}`);
        }
        if (!input.code) {
        ...
        const persistResult = this.providerService.setConfig("unix-gateway", ["couponCode"], tokenResult.data);
        if (!persistResult.success) {
            return (0, result_1.Err)(persistResult.error);
        }
        log_1.log.debug(`Unix Gateway OAuth exchange completed (state=${input.state})`);
        this.windowService?.focusMainWindow();
        return (0, result_1.Ok)(undefined);
    }
    async exchangeCodeForToken(code) {
        try {
            const response = await fetch(unixGatewayOAuth_1.UNIX_GATEWAY_EXCHANGE_URL, {
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                },
                body: (0, unixGatewayOAuth_1.buildExchangeBody)({ code }),
            });
            if (!response.ok) {
            ...
            if (!token) {
                return (0, result_1.Err)("Unix Gateway exchange response missing access_token");
            }
            return (0, result_1.Ok)(token);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Unix Gateway exchange failed: ${message}`);
        }
    }
...
exports.UnixGatewayOauthService = UnixGatewayOauthService;
...
//# sourceMappingURL=unixGatewayOauthService.js.map
...
exports.inlineSvgAsTextForProvider = inlineSvgAsTextForProvider;
const imageAttachments_1 = require("../../../common/constants/imageAttachments");
 ...
 */
function inlineSvgAsTextForProvider(messages, options) {
    const maxSvgTextChars = options?.maxSvgTextChars ?? imageAttachments_1.MAX_SVG_TEXT_CHARS;
    ...
    const result = messages.map((msg) => {
        ...
        const hasSvg = msg.parts.some((part) => part.type === "file" && normalizeMediaType(part.mediaType) === imageAttachments_1.SVG_MEDIA_TYPE);
        if (!hasSvg) {
            return msg;
        }
        didChange = true;
        ...
        for (const part of msg.parts) {
            if (part.type === "file" && normalizeMediaType(part.mediaType) === imageAttachments_1.SVG_MEDIA_TYPE) {
                try {
                    const svgText = decodeSvgDataUrlToUtf8(part.url, maxSvgTextBytes, maxSvgTextChars);
                    const textPart = {
                        type: "text",
                        text: `[SVG attachment converted to text (providers generally don't accept ${imageAttachments_1.SVG_MEDIA_TYPE} as an image input).]\n\n` +
                            `\`\`\`svg\n${svgText}\n\`\`\``,
                    };

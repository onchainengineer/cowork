...
describeIntegration("Runtime integration tests", () => {
  ...
  /**
   * LatticeSSHRuntime-specific tests
   *
   * Tests Coder-specific behavior like fork config updates.
   * Uses the same SSH fixture since LatticeSSHRuntime extends SSHRuntime.
   */
  describe("LatticeSSHRuntime workspace operations", () => {
    const srcBaseDir = "/home/testuser/src";

    // Create a LatticeSSHRuntime with mock LatticeService
    const createLatticeSSHRuntime = async () => {
      ...
      // Mock LatticeService with methods that LatticeSSHRuntime may call
      ...
      return new LatticeSSHRuntime(config, transport, mockLatticeService);
    };

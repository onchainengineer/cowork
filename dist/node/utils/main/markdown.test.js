...
Other content
`.trim();
            const result = (0, markdown_1.extractToolSection)(markdown, "bash");
...
`.trim();
        ...
        it("should return null for empty markdown", () => {
            expect((0, markdown_1.extractToolSection)("", "bash")).toBeNull();
        });
        it("should return null for empty tool name", () => {
            expect((0, markdown_1.extractToolSection)("# Tool: bash\nContent", "")).toBeNull();
        });
    });
...
Plan instructions
`.trim();
            expect((0, markdown_1.extractToolSection)(markdown, "bash")).toBe("Bash instructions");
            expect((0, markdown_1.extractToolSection)(markdown, "file_read")).toBe("File read instructions");
            expect((0, markdown_1.extractToolSection)(markdown, "propose_plan")).toBe("Plan instructions");
        });
...
`.trim();
    ...
    describe("tool names with underscores", () => {
        it("should handle file_read tool", () => {
            ...
            expect((0, markdown_1.extractToolSection)(markdown, "file_read")).toBe("Read instructions");
        });
        it("should handle file_edit_replace_string tool", () => {
            const markdown = "# Tool: file_edit_replace_string\nReplace instructions";
            expect((0, markdown_1.extractToolSection)(markdown, "file_edit_replace_string")).toBe("Replace instructions");
        });
    });
});
...
`.trim();
        const result = (0, markdown_1.stripScopedInstructionSections)(markdown);
...
`.trim();
    ...
    it("should handle empty markdown", () => {
        expect((0, markdown_1.stripScopedInstructionSections)("")).toBe("");
    });
});

...
class SSHRuntime extends RemoteRuntime_1.RemoteRuntime {
    ...
    async syncProjectToRemote(projectPath, workspacePath, initLogger, abortSignal) {
        ...
        await (0, gitBundleSync_1.syncProjectViaGitBundle)({
            ...
            createRemoteBundle: async ({ remoteBundlePath, initLogger, abortSignal }) => {
                ...
                const gitProc = (0, child_process_1.spawn)("git", ["-C", projectPath, "bundle", "create", "-", "--all"], {
    ...
    async initWorkspace(params) {
        ...
        try {
            ...
            const checkoutCmd = `git checkout ${streamUtils_1.shescape.quote(branchName)} 2>/dev/null || git checkout -b ${streamUtils_1.shescape.quote(branchName)} ${streamUtils_1.shescape.quote(newBranchBase)}`;
            ...
            if (skipInitHook) {
                ...
                if (hookExists) {
                    const muxEnv = { ...env, ...(0, initHook_1.getUnixEnv)(projectPath, "ssh", branchName) };
                    // Expand tilde in hook path (quoted paths don't auto-expand on remote)
                    const hookPath = (0, tildeExpansion_1.expandTildeForSSH)(`${workspacePath}/.unix/init`);
    ...
    async canFastForwardToOrigin(workspacePath, trunkBranch, initLogger, abortSignal) {
        try {
            ...
            const checkCmd = `git merge-base --is-ancestor ${streamUtils_1.shescape.quote(trunkBranch)} origin/${streamUtils_1.shescape.quote(trunkBranch)}`;
    ...
    async forkWorkspace(params) {
        ...
        try {
            ...
            const cloneResult = await (0, helpers_1.execBuffered)(this, `git clone --quiet ${sourceWorkspacePathArg} ${newWorkspacePathArg}`, {
                cwd: "/tmp",
                timeout: 300,
            });
            ...
            try {
                await (0, helpers_1.execBuffered)(this, `cd ${newWorkspacePathArg} && for branch in $(git for-each-ref --format='%(refname:short)' refs/remotes/origin/ | grep -v 'origin/HEAD'); do localname=\${branch#origin/}; git show-ref --verify --quiet refs/heads/$localname || git branch $localname $branch; done`, {
                    cwd: "/tmp",
                    timeout: 30,
                });
            }
            ...
            try {
                const originResult = await (0, helpers_1.execBuffered)(this, `git -C ${sourceWorkspacePathArg} remote get-url origin 2>/dev/null || true`, {
                    cwd: "/tmp",
                    timeout: 10,
                });
                const originUrl = originResult.stdout.trim();
                if (originUrl.length > 0) {
                    await (0, helpers_1.execBuffered)(this, `git -C ${newWorkspacePathArg} remote set-url origin ${streamUtils_1.shescape.quote(originUrl)}`, {
                        cwd: "/tmp",
                        timeout: 10,
                    });
                }
                else {
                    await (0, helpers_1.execBuffered)(this, `git -C ${newWorkspacePathArg} remote remove origin 2>/dev/null || true`, {
                        cwd: "/tmp",
                        timeout: 10,
                    });
                }

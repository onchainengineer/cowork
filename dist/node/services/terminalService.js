...
const commandDiscovery_1 = require("../../node/utils/commandDiscovery");
...
class TerminalService {
    ...
    outputEmitters = new Map();
    exitEmitters = new Map();
    ...
    serializeAddons = new Map();
    headlessOnDataDisposables = new Map();
    ...
    async create(params) {
        try {
            ...
            if (!workspaceMetadata.projectPath) {
                log_1.log.error("Workspace metadata missing projectPath", {
            ...
            const runtime = (0, runtimeFactory_1.createRuntime)(workspaceMetadata.runtimeConfig, {
            ...
            // 5. Create session
            const session = await this.ptyService.createSession(params, runtime, workspacePath, onData, onExit, workspaceMetadata.runtimeConfig);
            tempSessionId = session.sessionId;
            // Initialize emitters and headless terminal for state tracking
            this.outputEmitters.set(session.sessionId, new events_1.EventEmitter());
            this.exitEmitters.set(session.sessionId, new events_1.EventEmitter());
            ...
            const headless = new headless_1.Terminal({
                cols: params.cols,
                rows: params.rows,
            ...
            const disposeHeadlessOnData = headless.onData((data) => {
                ...
                try {
                    ...
                    log_1.log.debug("[TerminalService] Failed to forward terminal response", {
            ...
            log_1.log.error("Error creating terminal session:", err);
            throw err;
        }
    }
    close(sessionId) {
        try {
            ...
            log_1.log.error("Error closing terminal session:", err);
            throw err;
        }
    }
    resize(params) {
        try {
            ...
            log_1.log.error("Error resizing terminal:", err);
            throw err;
        }
    }
    sendInput(sessionId, data) {
        try {
            ...
            log_1.log.error(`Error sending input to terminal ${sessionId}:`, err);
            throw err;
        }
    }
    async openWindow(workspaceId, sessionId) {
        try {
            ...
            const isSSH = (0, runtime_1.isSSHRuntime)(runtimeConfig);
            const isDesktop = !!this.terminalWindowManager;
            if (isDesktop) {
                log_1.log.info(`Opening terminal window for workspace: ${workspaceId}${sessionId ? ` (session: ${sessionId})` : ""}`);
                await this.terminalWindowManager.openTerminalWindow(workspaceId, sessionId);
            }
            else {
                log_1.log.info(`Browser mode: terminal UI handled by browser for ${isSSH ? "SSH" : "local"} workspace: ${workspaceId}`);
            }
        }
        catch (err) {
            log_1.log.error("Error opening terminal window:", err);
            throw err;
        }
    }
    closeWindow(workspaceId) {
        try {
            ...
            log_1.log.error("Error closing terminal window:", err);
            throw err;
        }
    }
    /**
    ...
    async openNative(workspaceId) {
        try {
            ...
            const runtimeConfig = workspace.runtimeConfig;
            if ((0, runtime_1.isSSHRuntime)(runtimeConfig)) {
            ...
            else if ((0, runtime_1.isDockerRuntime)(runtimeConfig)) {
            ...
            else if ((0, runtime_1.isDevcontainerRuntime)(runtimeConfig)) {
            ...
            log_1.log.error(`Failed to open native terminal: ${message}`);
            throw err;
        }
    }
    /**
    ...
    async openNativeTerminalMacOS(config, sshArgs, logPrefix) {
        ...
        const terminal = await (0, commandDiscovery_1.findAvailableCommand)(["ghostty", "terminal"]);
        if (terminal === "ghostty") {
            const cmd = "open";
            let args;
            if (isSSH && sshArgs) {
            ...
            log_1.log.info(`Opening ${logPrefix}: ${cmd} ${args.join(" ")}`);
    ...
    async openNativeTerminalLinux(config, sshArgs, logPrefix) {
        ...
        if (availableTerminal) {
            ...
            log_1.log.info(`Opening ${logPrefix}: ${availableTerminal.cmd} ${availableTerminal.args.join(" ")}${cwdInfo}`);
            const child = (0, child_process_1.spawn)(availableTerminal.cmd, availableTerminal.args, {
            ...
            log_1.log.error("No terminal emulator found. Tried: " + terminals.map((t) => t.cmd).join(", "));
    ...
    async findAvailableTerminal(terminals) {
        for (const terminal of terminals) {
            if (await (0, commandDiscovery_1.isCommandAvailable)(terminal.cmd)) {
                return terminal;
            }
        }
        return null;

    ...
    (0, bun_test_1.describe)("new workspace mode", () => {
        (0, bun_test_1.it)("derives Coder name from branch name when not provided", async () => {
            ...
            if (result.success) {
                ...
                if (result.data.type === "ssh") {
                    (0, bun_test_1.expect)(result.data.coder?.workspaceName).toBe("unix-my-feature");
                    (0, bun_test_1.expect)(result.data.host).toBe("unix-my-feature.coder");
                }
            }
        });
        (0, bun_test_1.it)("converts underscores to hyphens", async () => {
            ...
            if (result.success && result.data.type === "ssh") {
                (0, bun_test_1.expect)(result.data.coder?.workspaceName).toBe("unix-my-feature-branch");
                (0, bun_test_1.expect)(result.data.host).toBe("unix-my-feature-branch.coder");
            }
        });
...
(0, bun_test_1.describe)("CoderSSHRuntime.deleteWorkspace", () => {
    ...
    (0, bun_test_1.it)("never calls coderService.deleteWorkspace when existingWorkspace=true", async () => {

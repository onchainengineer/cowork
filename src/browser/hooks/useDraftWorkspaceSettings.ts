...
export function useDraftWorkspaceSettings(
  ...
  const lastCoderConfig = readRuntimeConfig<LatticeWorkspaceConfig | null>(
    RUNTIME_MODE.SSH,
    "latticeConfig",
    null
  );
  ...
  const buildRuntimeForMode = (
    ...
    switch (mode) {
      ...
      case RUNTIME_MODE.SSH: {
        ...
        const effectiveHost =
          coderEnabled && latticeConfig && !sshHost.trim() ? LATTICE_RUNTIME_PLACEHOLDER : sshHost;

        return {
          mode: "ssh",
          host: effectiveHost,
          lattice: coderEnabled && latticeConfig ? latticeConfig : undefined,
        };
      }
    ...
    if (prevMode !== selectedRuntime.mode) {
      if (selectedRuntime.mode === RUNTIME_MODE.SSH) {
        ...
        const needsCoderRestore =
          selectedRuntime.lattice === undefined && lastCoderEnabled && lastCoderConfig;
        if (needsHostRestore || needsCoderRestore) {
          setSelectedRuntimeState({
            mode: RUNTIME_MODE.SSH,
            host: needsHostRestore ? lastSshHost : selectedRuntime.host,
            lattice: needsCoderRestore ? lastCoderConfig : selectedRuntime.lattice,
          });
        }
      }
  ...
  const setSelectedRuntime = (runtime: ParsedRuntime) => {
    ...
    if (runtime.mode === RUNTIME_MODE.SSH) {
      ...
      const coderEnabled = runtime.lattice !== undefined;
      setLastRuntimeConfig(RUNTIME_MODE.SSH, "coderEnabled", coderEnabled);
      if (runtime.lattice) {
        setLastRuntimeConfig(RUNTIME_MODE.SSH, "latticeConfig", runtime.lattice);
      }

...
class StreamManager extends events_1.EventEmitter {
    ...
    constructor(historyService, partialService, sessionUsageService) {
        super();
    ...
    createStreamResult(request, abortController, stepTracker) {
        return (0, ai_1.streamText)({
            ...
            ...(request.toolChoice
                ? { maxSteps: 1 }
                : { stopWhen: [(0, ai_1.stepCountIs)(100000), () => request.hasQueuedMessage?.() ?? false] }),
            // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
     ...
     */
    createStreamAtomically(workspaceId, streamToken, runtimeTempDir, runtime, messages, model, modelString, abortController, system, historySequence, messageId, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, workspaceName) {
                        ...
                        switch (part.type) {
                            ...
                            case "tool-result": {
                                ...
                                if (toolCall) {
                                    ...
                                    const strippedOutput = (0, internalToolResultFields_1.stripInternalToolResultFields)(stripEncryptedContent(part.output));
                                    toolCall.output = strippedOutput;
                                    // Use shared completion logic (await to ensure partial is flushed before event)
                                    await this.finishToolCall(workspaceId, streamInfo, toolCalls, part.toolCallId, part.toolName, strippedOutput);
                                }
                                break;
                            }
     ...
     */
    async startStream(workspaceId, messages, model, modelString, historySequence, system, runtime, messageId, abortSignal, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, providedStreamToken, hasQueuedMessage, workspaceName) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const streamInfo = this.createStreamAtomically(typedWorkspaceId, streamToken, runtimeTempDir, runtime, messages, model, modelString, streamAbortController, system, historySequence, messageId, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, workspaceName);
